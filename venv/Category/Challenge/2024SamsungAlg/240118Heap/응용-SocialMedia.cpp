...
struct Post{
	...
	Post(int pID, int id, int timestamp):pID(pID), uID(id),timestamp(timestamp),like(0) {}
};
vector<Post> posts;
map<int, vector<Post*>> userPost;
bool followed[MAX][MAX];
...
void init(int n)
{
	...
	for(int i=1;i<=N;i++){
		userPost.insert({1,{}});
		for(int j=1;j<=N;j++) followed[i][j]=false;
	}
}
void follow(int uID1, int uID2, int timestamp)
{
	followed[uID1][uID2]=true;
	followed[uID2][uID1]=true;
}

void makePost(int uID, int pID, int timestamp)
{
	Post tmp=Post(pID, uID, timestamp);
	posts.push_back(tmp);
	userPost[uID].push_back(&tmp);
}
...
int current;
int comp(Post *a, Post *b){
	int aTime=current - a->timestamp;
	int bTime=current - b->timestamp;
	if(aTime>=0 && bTime>=0){
...
{
	// timestamp, like, 1000s
	// priority_queue<Post, vector<Post>, comp> pq;
	vector<Post*> allPost;
	current=timestamp;

	for(int i=1;i<=N;i++){
		if(i==uID || followed[uID][i]) {
			for(auto p: userPost[i]){
				allPost.push_back(p);
			}
		}
	}

	sort(allPost.begin(), allPost.end(), comp);
	for(int i=0;i<10&&i<allPost.size();i++){
		pIDList[i]=allPost[i]->pID;
	}
}
